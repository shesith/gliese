    ...
    let dataTable = $('#datatable-cotize').DataTable({
        ...
        columns: [
            ...
            },    
            {
                ...
                render: function (data, type, row, meta) {
                    return (
                        '<button class="btn btn-sm btn-info btn-round btn-icon btn_update" data-process-key="'+ row.id_billingpersale +'">' +
                        feather.icons['edit'].toSvg({ class: 'font-small-4' }) +
                        '</button>'
                        + ' ' + 
                        '<button  class="btn btn-sm btn-danger btn-round btn-icon btn_delete" data-process-key="'+ row.id_billingpersale +'">' +
        ...
        buttons: functions.custom_buttons_datatable([7], '#create_billingpersale_modal'), // -- Number of columns
        language: {
...
$(document).on('click', '.reset', function() {
    // --
    $('#update_billingpersale_form').validate().resetForm();
})
